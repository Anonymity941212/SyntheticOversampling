...
def get_scores(y_true: np.array, y_pred: np.array,
    ...
    if not bootstrap:
        ...
        for name, metric in [('acc', accuracy_score), ('bacc', balanced_accuracy_score),
...
def make_num_dist_plots(X_real: np.array, X_fake: np.array,
    ...
    for idx, ax in enumerate(axes.flatten()):
        sns.kdeplot(X_real[:real_size, idx], label='real', ax=ax, shade=True, legend=False, bw_adjust=.25)
        sns.kdeplot(X_fake[:fake_size, idx], label='fake', ax=ax, shade=True, legend=False, bw_adjust=.25)
        ax.set_yticks([])
        ax.set_xticks([0, 1])
        if num_cols is not None:
